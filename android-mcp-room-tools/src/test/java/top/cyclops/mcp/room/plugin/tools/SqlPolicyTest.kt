...
class SqlPolicyTest {
    ...
    @Test
    fun `check returns affected table names for write statements`() {
        assertAffectedTable("users", "INSERT INTO users(name) VALUES ('Ada')")
        assertAffectedTable("users", "INSERT OR REPLACE INTO main.users(name) VALUES ('Ada')")
        assertAffectedTable("users", "UPDATE users SET name = 'Ada'")
        assertAffectedTable("users", "DELETE FROM users WHERE id = 1")
        assertAffectedTable("users", "REPLACE INTO `users`(id, name) VALUES (1, 'Ada')")
        assertAffectedTable("user table", "UPDATE \"user table\" SET name = 'Ada'")
        assertAffectedTable("users", "DROP TABLE IF EXISTS [users]")
    }

    private fun assertAffectedTable(expected: String, sql: String) {
        val result = SqlPolicy.check(sql, SqlPolicyConfig(allowWrites = true))

        assertTrue(result is SqlPolicyResult.Allowed)
        assertEquals(setOf(expected), (result as SqlPolicyResult.Allowed).affectedTables)
    }
}
